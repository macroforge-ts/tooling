...
type EffectSchemaEncoded<T> = T extends {
  ...
  : T extends { constructor: infer C }
    ? C extends { Encoded: infer InstanceEncoded } ? InstanceEncoded
    : T
  : T;
type ResolveOption<T> = T extends EffectOption.Option<infer Value> ? Value : T;
type NormalizeDuration<T> = T extends Duration ? DurationEncoded | DurationTuple
  : T;
type NormalizeTarget<T> = NormalizeDuration<
  ResolveOption<EffectSchemaEncoded<T>>
>;

// ---------------- Core helpers ----------------

// ---- callable / constructable detection (lint-clean) ----
type IsCallable<T> = T extends (...args: Array<never>) => unknown ? true
  : false;
type IsConstructable<T> = T extends
  abstract new (...args: Array<never>) => unknown ? true : false;

// ---- non-function "object" without using the 'object' type ----
type HasPipe<T> = ResolveOption<EffectSchemaEncoded<T>> extends {
  pipe: (...args: Array<unknown>) => unknown;
} ? true
  : false;

type HasSymbolKey<T> =
  Extract<keyof ResolveOption<EffectSchemaEncoded<T>>, symbol> extends never
    ? false
    : true;

type IsNonFunctionRecord<T> = IsCallable<T> extends true ? false
  : IsConstructable<T> extends true ? false
  : HasPipe<T> extends true ? false
  : HasSymbolKey<T> extends true ? false
  : NormalizeTarget<T> extends object
    ? keyof NormalizeTarget<T> extends never ? false
    : true
  : false;

// ---- cycle tracker ----
// IMPORTANT: use a NON-distributive check so unions don't slip through.
type SeenHas<Seen, T> = [T] extends [never] ? false
  : [T] extends [Seen] ? true
  : false;
type SeenAdd<Seen, T> = Seen | T;

type _IsTuple<P extends ReadonlyArray<unknown>> = number extends P["length"]
  ? false
  : true;
type TupleKeys<P extends ReadonlyArray<unknown>> = {
  [I in Exclude<keyof P, keyof ReadonlyArray<unknown>>]: I extends
...
type StopOnSeen<T, Seen, Fallback, Next> = SeenHas<Seen, T> extends true
  ? Fallback
  : Next;

type OnCycleKeys<T> = NormalizeTarget<T> extends infer Base
  ? Base extends ReadonlyArray<unknown> ? number
  : IsNonFunctionRecord<Base> extends true ? keyof Base
  : never
  : never;
...
type ValidKeyAt<T, Seen = never> = Base<T> extends infer B
  ? [B] extends [Seen] ? never // Cycle detected - stop here
  : B extends ReadonlyArray<unknown> ? number
  : IsNonFunctionRecord<B> extends true ? PropertyKeyOf<B>
  : never
  : never;
...
type TypeAt<T, K extends Key> = Base<T> extends infer B
  ? K extends number ? B extends ReadonlyArray<infer E> ? E
    : never
...
> = [D] extends [
  ...
  : P extends readonly [] ? readonly []
  : P extends
    readonly [infer H extends Key, ...infer R extends ReadonlyArray<Key>]
    ? H extends ValidKeyAt<T, Seen> ? R extends readonly [] ? readonly [H]
...
type PropertyKeyOf<T> = T extends infer O
  ? O extends object ? Extract<keyof O, string | number>
  : never
  : never;

// Internal alias with default depth for backwards-compatible internal usages
type DeepPathInternal<T, D extends number = 5, Seen = never> = DeepPath<
  T,
  D,
  Seen
>;
...
> = NormalizeTarget<T> extends infer Base ? StopOnSeen<
    Base,
    Seen,
    unknown,
    P extends readonly [] ? Base
  ...
  >
  : never;
export type ResolveDeepPath<_T, P, _Seen = never> = P extends ReadonlyArray<Key>
  ? P
  : never;
type DeepValueResult<T, P, Seen> = DeepValueInternal<
  T,
  Seen,
  ResolveDeepPath<T, P, Seen>
>;
...
// ---- DeepValueUnion ----
export type DeepValueUnion<T, Seen = never> = NormalizeTarget<T> extends
  infer Base ? SeenHas<Seen, Base> extends true ? never
  : Base extends ReadonlyArray<infer E> ? DeepValueUnion<E, SeenAdd<Seen, Base>>
  : Base extends Array<infer E> ? DeepValueUnion<E, SeenAdd<Seen, Base>>
...
// ---- DeepLeafValueUnion ----
export type DeepLeafValueUnion<T, Seen = never> = NormalizeTarget<T> extends
  infer Base ? SeenHas<Seen, Base> extends true ? never
  : Base extends ReadonlyArray<infer E>
    ? DeepLeafValueUnion<E, SeenAdd<Seen, Base>>
  : Base extends Array<infer E> ? DeepLeafValueUnion<E, SeenAdd<Seen, Base>>
  : IsNonFunctionRecord<Base> extends true ? {
...
export type DeepKey<T, Seen = never, D extends number = 3> =
  NormalizeTarget<T> extends infer Base ? StopOnSeen<
      Base,
      Seen,
      D extends 0 ? OnCycleKeys<Base>
 ...
 */
export type DeepPath<T, D extends number = 5, _Seen = never> =
  NormalizeTarget<T> extends infer Base ? D extends 0 ? []
    : [] | DeepPathNonEmpty<Base, D>
    : never;

type DeepPathNonEmpty<T, D extends number = 5> = NormalizeTarget<T> extends
  infer Base ? Base extends unknown ? Base extends Primitive ? never
    : Base extends ReadonlyArray<infer Elem> ? _IsTuple<Base> extends true ? {
    ...
    : IsNonFunctionRecord<Base> extends true ? {
        [K in PropertyKeyOf<Base>]-?: IsCallable<Base[K]> extends true ? never
          : IsConstructable<Base[K]> extends true ? never
...
type NormalizeDepth<Depth extends number, Fallback extends number> =
  Depth extends keyof DepthMap ? Depth
    : Fallback;

type PathDepth<
  P extends ReadonlyArray<unknown>,
  Fallback extends number = 5,
> = number extends P["length"] ? Fallback
  : NormalizeDepth<P["length"], Fallback>;
 ...
 */
export type DeepPathFor<
  T,
  P extends ReadonlyArray<Key>,
  Fallback extends number = 3,
> = Extract<
  DeepPathInternal<T>,
  ToReadonlyTuple<P>
> extends never ? DeepPath<T, PathDepth<P, Fallback>>
  : Extract<DeepPathInternal<T>, ToReadonlyTuple<P>>;

// Export step-narrowing types for external use
export type { NarrowPath, TypeAt, ValidKeyAt };
...
export type ResolvedDeepValue = DeepValue<
  Record<Key, unknown>,
  ResolvedDeepPath
>;
...
type IsLeaf<T, Seen = never> = NormalizeTarget<T> extends infer Base
  ? SeenHas<Seen, Base> extends true ? true
  : Base extends ReadonlyArray<unknown> ? false
  : Base extends Array<unknown> ? false
  : IsNonFunctionRecord<Base> extends true ? false
  : true
  : true;

type DeepLeafPathHelper<T, Seen = never> = NormalizeTarget<T> extends infer Base
  ? SeenHas<Seen, Base> extends true ? never
  : Base extends ReadonlyArray<infer E>
...
export type DeepLeafPath<T> =
  & (IsLeaf<T> extends true ? readonly [] : DeepLeafPathHelper<T>)
  & DeepPathInternal<T>;

// ---- DeepProp (inherits non-distributive SeenHas fix) ----
export type DeepProp<T, K extends PropertyKey, Seen = never> =
  NormalizeTarget<T> extends infer Base
    ? SeenHas<Seen, Base> extends true ? never
    : Base extends ReadonlyArray<infer E> ? DeepProp<E, K, SeenAdd<Seen, Base>>
    : Base extends Array<infer E> ? DeepProp<E, K, SeenAdd<Seen, Base>>
    : IsNonFunctionRecord<Base> extends true ?
...
export function setNestedValue<T extends object>(
  ...
  if (finalKey && Array.isArray(cursor)) {
    if (!isIndex(finalKey)) {
      throw new Error(
        `Cannot use string key '${finalKey}' on array at final path index ${
          String(lastIdx)
        }`,
      );
    }
...
>(
  ...
  if (!Array.isArray(path)) {
    throw new TypeError(
      `Invalid argument: 'path' must be an array. Received: ${typeof path}`,
    );
  }
  if (path.length === 0) {
 ...
 * ```
 */
export function isResolvedDeepValue<
  T extends object,
  const P extends ResolvedDeepPath,
>(
  obj: T,
  path: P,
  value: unknown,
  ...
  const existingValue = getNestedValue(
    obj,
    path as unknown as DeepPathInternal<T>,
  );
