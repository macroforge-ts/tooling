...
type HasSymbolKey<T> = Extract<keyof ResolveOption<EffectSchemaEncoded<T>>, symbol> extends never
    ? false
    : true;
...
type IsNonFunctionRecord<T> = IsCallable<T> extends true ? false
    ...
    : NormalizeTarget<T> extends object ? keyof NormalizeTarget<T> extends never ? false
        : true
    : false;
...
type _IsTuple<P extends ReadonlyArray<unknown>> = number extends P['length'] ? false
    : true;
type TupleKeys<P extends ReadonlyArray<unknown>> = {
    [I in Exclude<keyof P, keyof ReadonlyArray<unknown>>]: I extends `${infer N extends number}` ? N
        : I extends number ? I
        : never;
...
type StopOnSeen<T, Seen, Fallback, Next> = SeenHas<Seen, T> extends true ? Fallback
    : Next;
 ...
 */
type ValidKeyAt<T, Seen = never> = Base<T> extends infer B ? [B] extends [Seen] ? never // Cycle detected - stop here
...
// ----------------- DeepValue -----------------
type PropertyKeyOf<T> = T extends infer O ? O extends object ? Extract<keyof O, string | number>
    : never
    : never;
...
> = NormalizeTarget<T> extends infer Base ? StopOnSeen<
        ...
        P extends readonly [] ? Base
            ...
            : P[number] extends PropertyKeyOf<Base> ? {
                    ...
                    > extends infer TK
                        ? TK extends unknown
                            ? K extends keyof TK ? DeepValueInternal<TK[K], SeenAdd<Seen, TK>, P>
                            : never
                        : never
                        : never;
    ...
    >
    : never;
export type ResolveDeepPath<_T, P, _Seen = never> = P extends ReadonlyArray<Key> ? P
    : never;
...
export type DeepLeafValueUnion<T, Seen = never> = NormalizeTarget<T> extends infer Base
    ? SeenHas<Seen, Base> extends true ? never
    : Base extends ReadonlyArray<infer E> ? DeepLeafValueUnion<E, SeenAdd<Seen, Base>>
...
type DeepPathNonEmpty<T, D extends number = 5> = NormalizeTarget<T> extends infer Base
    ? Base extends unknown ? Base extends Primitive ? never
...
type DeepLeafPathHelper<T, Seen = never> = NormalizeTarget<T> extends infer Base
    ...
    : Base extends Array<infer E> ? readonly [number, ...DeepLeafPathHelper<E, SeenAdd<Seen, Base>>]
    : IsNonFunctionRecord<Base> extends true ? {
            [K in keyof Base]-?: IsLeaf<Base[K], SeenAdd<Seen, Base>> extends true ? readonly [K]
...
export const isObjectLike = (
    value: unknown
): value is Record<PropertyKey, unknown> => typeof value === 'object' && value !== null;
