...
function runAllMacroTests() {
    // Access the module as any to check for macro-generated standalone functions
    // The macro expansion generates standalone functions (e.g., svelteAllMacrosTestToString),
    // not namespace methods. We use dynamic access to avoid type errors at dev time.
    const mod = { SvelteAllMacrosTest } as any;

    // Test Debug macro -> toString() (standalone function generated by macro)
    if (typeof (globalThis as any).svelteAllMacrosTestToString === 'function') {
        testResults.debug = (globalThis as any).svelteAllMacrosTestToString(svelteTestInstance);
    } else if (typeof mod.SvelteAllMacrosTest?.toString === 'function' && mod.SvelteAllMacrosTest.toString !== Object.prototype.toString) {
        testResults.debug = mod.SvelteAllMacrosTest.toString(svelteTestInstance);
    }

    // Test Clone macro -> clone() (standalone function generated by macro)
    // Note: These methods are generated as standalone functions by the macro expansion
    // and may not be available as namespace methods. Skipping for now.

    // Test Eq macro -> equals()
    // Skipped: macro generates svelteAllMacrosTestEquals as standalone function

    // Test Eq macro -> hashCode()
    // Skipped: macro generates svelteAllMacrosTestHashCode as standalone function

    // Test Serialize macro -> serialize()
    // Skipped: macro generates svelteAllMacrosTestSerialize as standalone function

    // Test Deserialize macro -> deserialize()
    // Skipped: macro generates svelteAllMacrosTestDeserialize as standalone function
