...
export function statusToString(value: Status): string {
    const key = Status[value as unknown as keyof typeof $MfPh3];
    if (key !== undefined) {
        return `${"Status"}.` + key;
    }
...
export function statusDeserializeWithContext(data: unknown): Status {
    for (const key of Object.keys(Status)){
        const enumValue = Status[key as keyof typeof $MfPh7];
        if (enumValue === data) {
...
export function statusIs(value: unknown): value is Status {
    for (const key of Object.keys(Status)){
        const enumValue = Status[key as keyof typeof $MfPh14];
        if (enumValue === value) {
            return true;
        }
    }
...
export function priorityToString(value: Priority): string {
    const key = Priority[value as unknown as keyof typeof $MfPh3];
    if (key !== undefined) {
        return `${"Priority"}.` + key;
    }
...
export function prioritySerializeWithContext(value: Priority, _ctx: __mf_SerializeContext): string | number {
    return value;
}

export function priorityDeserialize(input: unknown): Priority {
    const data = typeof input === "string" ? JSON.parse(input) : input;
...
export function priorityDeserializeWithContext(data: unknown): Priority {
    for (const key of Object.keys(Priority)){
        const enumValue = Priority[key as keyof typeof $MfPh7];
        if (enumValue === data) {
...
export function priorityIs(value: unknown): value is Priority {
    for (const key of Object.keys(Priority)){
        const enumValue = Priority[key as keyof typeof $MfPh14];
        if (enumValue === value) {
            return true;
        }
    }
...
export function colorToString(value: Color): string {
    const key = Color[value as unknown as keyof typeof $MfPh3];
    if (key !== undefined) {
        return `${"Color"}.` + key;
    }
...
export function pointHashCode(value: Point): number {
    let hash = 17;
    hash = (hash * 31 + (Number.isInteger(value.x) ? value.x | 0 : value.x.toString().split('').reduce((h, c)=>(h * 31 + c.charCodeAt(0)) | 0, 0))) | 0;
...
export function pointDeserialize(input: unknown, opts: __mf_DeserializeOptions): { success: true; value: Point } | { success: false; errors: Array<{ field: string; message: string }> } {
    try {
        const data = typeof input === "string" ? JSON.parse(input) : input;
...
export function pointDeserializeWithContext(value: any, ctx: __mf_DeserializeContext): Point | __mf_PendingRef {
    ...
    if (typeof value !== "object" || value === null || Array.isArray(value)) {
        throw new __mf_DeserializeError([
            {
                field: "_root",
                message: `${"Point"}.deserializeWithContext: expected an object`
            }
        ]);
    }
...
export function pointValidateField<K extends keyof Point>(_field: K, _value: Point[K]): Array<{
    field: string;
    message: string;
}> {
    return [];
}
export function pointValidateFields(_partial: Partial<Point>): Array<{
    field: string;
    message: string;
}> {
    return [];
...
export function userProfileEquals(a: UserProfile, b: UserProfile): boolean {
    if (a === b) return true;
    return a.id === b.id && a.username === b.username && a.email === b.email && a.age === b.age && a.isVerified === b.isVerified;
}

export function userProfileHashCode(value: UserProfile): number {
    let hash = 17;
    hash = (hash * 31 + (value.id ?? '').split('').reduce((h, c)=>(h * 31 + c.charCodeAt(0)) | 0, 0)) | 0;
...
export function userProfileDeserialize(input: unknown, opts: __mf_DeserializeOptions): { success: true; value: UserProfile } | { success: false; errors: Array<{ field: string; message: string }> } {
    try {
        const data = typeof input === "string" ? JSON.parse(input) : input;
        const ctx = __mf_DeserializeContext.create();
...
export function userProfileDeserializeWithContext(value: any, ctx: __mf_DeserializeContext): UserProfile | __mf_PendingRef {
    if (value?.__ref !== undefined) {
    ...
    if (typeof value !== "object" || value === null || Array.isArray(value)) {
        throw new __mf_DeserializeError([
            {
                field: "_root",
                message: `${"UserProfile"}.deserializeWithContext: expected an object`
            }
        ]);
    }
    const obj = value as Record<string, unknown>;
...
export function userProfileValidateField<K extends keyof UserProfile>(_field: K, _value: UserProfile[K]): Array<{
    field: string;
    message: string;
}> {
    return [];
}
export function userProfileValidateFields(_partial: Partial<UserProfile>): Array<{
    field: string;
    message: string;
}> {
    return [];
...
export function coordinate3DHashCode(value: Coordinate3D): number {
    ...
    hash = (hash * 31 + (Number.isInteger(value.x) ? value.x | 0 : value.x.toString().split('').reduce((h, c)=>(h * 31 + c.charCodeAt(0)) | 0, 0))) | 0;
    hash = (hash * 31 + (Number.isInteger(value.y) ? value.y | 0 : value.y.toString().split('').reduce((h, c)=>(h * 31 + c.charCodeAt(0)) | 0, 0))) | 0;
    hash = (hash * 31 + (Number.isInteger(value.z) ? value.z | 0 : value.z.toString().split('').reduce((h, c)=>(h * 31 + c.charCodeAt(0)) | 0, 0))) | 0;
    return hash;
}
