...
pub struct BaseControllerOptions {
    ...
    /// Transform function to apply to input values.
    /// Can be a preset name ("uppercase", "lowercase", "trim") or a custom function name.
...
pub fn parse_union_config(
    ...
    match union_kind {
        ...
        UnionKind::TypeRefUnion => {
            // Check for parenthesized unions - these are not supported because
            ...
            if !parenthesized.is_empty() {
                return Err(format!(
                    "Parenthesized union expressions ({}) are not supported. \
                     Formatters cannot preserve doc comments inside parentheses. \
