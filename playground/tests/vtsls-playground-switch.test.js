...
const _PLUGIN_NAME = '@macroforge/typescript-plugin';
const PLUGIN_PATH = path.join(PLUGIN_DIST, 'index.js');

const FILES = [
    path.resolve(REPO_ROOT, 'playground/svelte/src/lib/demo/macro-user.ts'),
    path.resolve(REPO_ROOT, 'playground/svelte/src/lib/demo/field-controller.ts')
];
...
test('vtsls playground switch', async (t) => {
    ...
    const _sendChange = (uri, text, version) =>
        server.send(
    ...
    const errorResponses = server.messages.filter((m) => m && m.error && m.error !== null);
    assert.strictEqual(
