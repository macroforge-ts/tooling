...
describe("Custom Validators", () => {
  ...
  describe("CustomNumberValidator (isEven)", () => {
    ...
    test("accepts negative even number", () => {
      const result = mod.CustomNumberValidator.deserialize(JSON.stringify({ evenNumber: -4 }));
      assertValidationSuccess(result, "evenNumber");
    });
    ...
    test("rejects negative odd number", () => {
      const result = mod.CustomNumberValidator.deserialize(JSON.stringify({ evenNumber: -5 }));
      assertValidationError(result, "evenNumber", "");
    });
  });
  ...
  describe("CustomStringValidator (isValidUsername)", () => {
    test("accepts valid username", () => {
      const result = mod.CustomStringValidator.deserialize(JSON.stringify({ username: "john_doe" }));
      assertValidationSuccess(result, "username");
    });

    test("accepts username with numbers", () => {
      const result = mod.CustomStringValidator.deserialize(JSON.stringify({ username: "user123" }));
      assertValidationSuccess(result, "username");
    });

    test("accepts 3 character username", () => {
      const result = mod.CustomStringValidator.deserialize(JSON.stringify({ username: "abc" }));
      assertValidationSuccess(result, "username");
    });

    test("accepts 16 character username", () => {
      const result = mod.CustomStringValidator.deserialize(JSON.stringify({ username: "a".repeat(16) }));
      assertValidationSuccess(result, "username");
    });

    test("rejects 2 character username", () => {
      const result = mod.CustomStringValidator.deserialize(JSON.stringify({ username: "ab" }));
      assertValidationError(result, "username", "");
    });

    test("rejects 17 character username", () => {
      const result = mod.CustomStringValidator.deserialize(JSON.stringify({ username: "a".repeat(17) }));
      assertValidationError(result, "username", "");
    });

    test("rejects username with spaces", () => {
      const result = mod.CustomStringValidator.deserialize(JSON.stringify({ username: "john doe" }));
      assertValidationError(result, "username", "");
    });

    test("rejects username starting with number", () => {
      const result = mod.CustomStringValidator.deserialize(JSON.stringify({ username: "1john" }));
      assertValidationError(result, "username", "");
    });

    test("rejects uppercase username", () => {
      const result = mod.CustomStringValidator.deserialize(JSON.stringify({ username: "JohnDoe" }));
      assertValidationError(result, "username", "");
    });
  });
  ...
  describe("CustomWithMessageValidator", () => {
    test("accepts even number", () => {
      const result = mod.CustomWithMessageValidator.deserialize(JSON.stringify({ evenNumber: 4 }));
      assertValidationSuccess(result, "evenNumber");
    });

    test("rejects odd with custom message", () => {
      const result = mod.CustomWithMessageValidator.deserialize(JSON.stringify({ evenNumber: 3 }));
