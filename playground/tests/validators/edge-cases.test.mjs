...
describe('Edge Cases', () => {
    ...
    describe('MultipleValidators', () => {
        ...
        test('accepts value at max length boundary', () => {
            const result = mod.MultipleValidatorsTest.deserialize(
                JSON.stringify({ text: 'a'.repeat(100) })
            );
            assertValidationSuccess(result, 'text');
        });
        ...
        test('rejects string exceeding max length', () => {
            const result = mod.MultipleValidatorsTest.deserialize(
                JSON.stringify({ text: 'a'.repeat(101) })
            );
    ...
    describe('MixedValidators', () => {
        test('accepts valid email', () => {
            const result = mod.MixedValidatorsTest.deserialize(
                JSON.stringify({ email: 'user@domain.com' })
            );
            assertValidationSuccess(result, 'email');
        });
    ...
    describe('CombinedStringValidators', () => {
        ...
        test('accepts maximum length username', () => {
            const result = mod.CombinedStringValidatorsTest.deserialize(
                JSON.stringify({ username: 'a'.repeat(20) })
            );
            assertValidationSuccess(result, 'username');
        });
    ...
    describe('CombinedNumberValidators', () => {
        test('accepts valid positive integer', () => {
            const result = mod.CombinedNumberValidatorsTest.deserialize(
                JSON.stringify({ score: 500 })
            );
